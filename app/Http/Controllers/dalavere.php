...
<script>
    $(document).ready(function() {
        ...
        function listCarPartsCatalog(response) {
            ...
            response.forEach(function (car) {
                var groupName = car.group_name;
                var groupItem = $(`<ul class="list-group-item1 border p-2">${groupName}</ul>`);
                modalList.append(groupItem);

                var subGroupList = $('<ul class="sub-group-list"></ul>');
                groupItem.append(subGroupList);

                var subGroupItem = $(`<li class="list-group-item31">${car.subGroupName}</li>`);
                subGroupList.append(subGroupItem);
        ...
        function parameters(car) {
            ...
            console.log("Tıklanan Part ID:", car.part_group_id);
            console.log("Part Information:", car);

            var partItem = $('<div class="part-item mb-3">');
            partItem.append(`<strong>${car.part_name}</strong><br>`);

            // Görsel yolu düzenleme
            var imagePath = car.img.replace('/r/250x250', '');
            partItem.append(`<img src="${imagePath}" alt="${car.part_name}" class="custom-img1" id="${car.part_group_id}" style="border: 1px solid black; border-radius: 10px;" />`);
            ...
            $(document).on('click', 'img.custom-img1', function () {
                ...
                console.log("Part Group URL:", url);
    ...
    public function carGroupList(Request $request)
    {
        ...
        $results = DB::table('catalog_sub_groups as cs1')
    ->join('catalog_sub_groups as cs2', 'cs1.group_id', '=', 'cs2.parent_id')
    ->join('catalog_groups as cs5', 'cs5.group_id', '=', 'cs2.parent_id')
    ->join('catalog_car_schemas as cs3', 'cs3.group_id', '=', 'cs2.group_id')
    ->where('cs1.car_id', $car_id)
    ->select(
        'cs2.name as subGroupName',
        'cs3.part_name as part_name',
        'cs3.part_group_id as part_id',
        'cs3.img',
        'cs5.car_id',
        'cs5.name as group_name'
    )
    ->distinct()
    ->get();

        $results = $results->unique('part_group_id')->values();
